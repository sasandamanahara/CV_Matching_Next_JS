...
export default function ApplicantList() {
  ...
  const { data} = useSWR(`/get-specified-job-applicants`, () =>
    get_specified_job_applicants(id)
  );

  useEffect(() => {
    if (data) setJobApplicantsDetails(data?.data);
  }, [data]);
...
<div className="flex flex-col">
        ...
        <main className="flex flex-1 flex-col gap-4 p-4 lg:gap-6 lg:p-6">
          ...
          <h1 className="text-lg font-semibold md:text-2xl">All Applicants for {JobApplicantsDetails && JobApplicantsDetails[0]?.jobId?.title} Position</h1>
        ...
        <div className="flex flex-col md:flex-row md:justify-between mb-4">
          <h2 className="text-lg font-bold text-orange-400 mb-2 md:mb-0">Total Applicants: {JobApplicantsDetails && JobApplicantsDetails.length}
</h2>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
           ...
           <tbody>
           {JobApplicantsDetails && JobApplicantsDetails.map(applicant => (
        <tr key={applicant._id}>
          ...
          <td className="px-3 py-2 text-xs bg-white border-b border-gray-200">
            <span className="relative inline-block px-2 py-1 text-xs font-semibold leading-tight text-green-900">
              ...
              {applicant.status === 'pending' ? 'Active' : applicant.status}
              </span>
            </span>
          </td>
          <td className="px-3 py-2 text-xs bg-white border-b border-gray-200">
