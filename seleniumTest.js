...
async function testLoginPage() {
  ...
  try {
    ...
    try {
      ...
      const isDashboardDisplayed = await dashboardHeading.isDisplayed();
      console.log("Dashboard Heading Displayed:", isDashboardDisplayed);
    } catch (error) {
      console.error("Failed to find 'DashBoard' heading:", error);
      throw error;
    }
    ...
    const makeCVButton = await driver.findElement(
      By.xpath("//button[contains(text(), 'Make CV')]")
    );
    await sleep(2000);
